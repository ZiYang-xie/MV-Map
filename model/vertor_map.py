...
from dataloader.constant import CLASS2LABEL
...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, patch_size, canvas_size, location, center):
        self.patch_size = patch_size
        self.canvas_size = canvas_size
        map_pose = center[:3,3][:2]
        yaw = Rotation.from_matrix(center[:3,:3]).as_euler('zxy')[0]
...
def generate_gt_hdmap(nusc, rec, vector_map, patch_size, canvas_size, NUM_CLASSES=3):
...
if __name__ == '__main__':
    ...
    vector_map = VectorizedLocalMap(nusc_dataroot)
    patch_size = [100,100] # 100m x 100 m
    voxel_size = 0.15
    canvas_size = [int(patch_size[0]/voxel_size),int(patch_size[1]/voxel_size)]
    semantic_map = generate_gt_hdmap(nusc, rec, vector_map, patch_size, canvas_size)