...
from utils.misc import points_to_voxels
...
class PointPillarEncoder(nn.Module):
  def __init__(self, cfg):
    ...
    # [min_range, max_range, voxel_size]
    out_channel = cfg.pc_encoder_config.out_channel
    patch_size = cfg.main_cfg.data_config.patch_size
    y_range = cfg.main_cfg.data_config.y_range
    voxel_h_sz = 1 / cfg.main_cfg.data_config.hight_resolution_scale
    voxel_size =  cfg.main_cfg.data_config.gt_voxel_size
    self.xbound = [patch_size[0], patch_size[1], voxel_size]
    self.ybound = [patch_size[0], patch_size[1], voxel_size]
    self.zbound = [y_range[0], y_range[1], voxel_h_sz]

    self.pn = PointNet(15, 64)
    self.block1 = PillarBlock(384, dims=64, num_layers=2, stride=1)
    self.block2 = PillarBlock(64, dims=128, num_layers=3, stride=2)
    self.block3 = PillarBlock(128 , 256, num_layers=3, stride=2)
    self.up1 = nn.Sequential(
      nn.Conv2d(64, 64, 3, padding=1, bias=False),
